...
public class Towers {
	...
	public void move(int source, int destination) {
		...
		int destinationTopRingIndex = heights[destinationIndex] - 1;
		int sourceTopRingIndex = heights[sourceIndex] - 1;
		towers[destinationIndex][destinationTopRingIndex + 1] = towers[sourceIndex][sourceTopRingIndex];
	...
	public void printTowers() {
		...
		int[] rings = { h1, h2, h3 };
		for (int outer = 1; outer <= maxHeight; outer++) {
			for (int i = 0; i < 3; i++) {
				if (spaces[i] > 0) {
					print(' ', maxHeight);
					System.out.print('|');
